function findCaliforniaCafes(searchTerm) {
    ...
    //First we will filter cafes based on the search term
    const matchingCafes = cafes.filter(cafe => cafe.name.toLowerCase().includes(searchTerm.toLowerCase()));
  
    // Here, we are mapping the matching cafes to their corresponding places
    const results = matchingCafes.map(matchingCafe => {
      const matchingPlace = places.find(place => place.id === matchingCafe.place_id);
      return { ...matchingCafe, ...matchingPlace };
    });
  ...
  // Example
  const results = findCaliforniaCafes('California');
  console.log(results);
